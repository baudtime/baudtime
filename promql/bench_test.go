...
func BenchmarkRangeQuery(b *testing.B) {
	dataNodeAPI := backend.NewFanout(nil)
	defer dataNodeAPI.Close()
	engine := NewEngine(nil, 10, 100*time.Second)
	...
	for s := 0; s < numIntervals; s++ {
		a, err := dataNodeAPI.Appender()
		if err != nil {
			b.Fatal(err)
		}
		ts := int64(s * 10000) // 10s interval.
		for _, metric := range metrics {
			err := a.Add(util.LabelsToProto(metric), ts, float64(s), metric.Hash())
			if err != nil {
				b.Fatal(err)
			}
		}
