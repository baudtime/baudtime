...
// against a test dataNodeAPI.
type Test struct {
	testutil.T

	cmds []testCommand

	dataNodeAPI backend.Backend
...
func (t *Test) Queryable() backend.Queryable {
	return t.dataNodeAPI
}
...
// Storage returns the test's dataNodeAPI.
func (t *Test) Storage() backend.Backend {
	return t.dataNodeAPI
}
...
// append the defined time series to the dataNodeAPI.
func (cmd *loadCmd) append(a backend.Appender) error {
	for h, smpls := range cmd.defs {
		...
		for _, s := range smpls {
			if err := a.Add(lset, s.T, s.V, m.Hash()); err != nil {
				return err
			}
		}
	}
...
// clearCmd is a command that wipes the test's dataNodeAPI state.
...
func (t *Test) exec(tc testCommand) error {
	...
	case *loadCmd:
		app, err := t.dataNodeAPI.Appender()
		if err != nil {
			return err
		}
	...
	case *evalCmd:
		...
		q, _ = t.queryEngine.NewRangeQuery(t.dataNodeAPI, cmd.expr, cmd.start.Add(-time.Minute), cmd.start.Add(time.Minute), time.Minute)
...
// clear the current test dataNodeAPI of all inserted samples.
func (t *Test) clear() {
	if t.dataNodeAPI != nil {
		if err := t.dataNodeAPI.Close(); err != nil {
			t.T.Fatalf("closing test dataNodeAPI: %s", err)
		}
	}
	if t.cancelCtx != nil {
		t.cancelCtx()
	}
	t.dataNodeAPI = backend.NewFanout(nil)

	t.queryEngine = NewEngine(nil, 20, 10*time.Second)
